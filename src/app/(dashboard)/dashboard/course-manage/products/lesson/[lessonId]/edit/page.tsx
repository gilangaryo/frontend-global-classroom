...
import TagsInput from "@/app/(dashboard)/dashboard/components/form/TagsInput";
            ...
            try {
                ...
                if (lessonData.status === 'success' && lessonData.data) {
                    ...
                    setFormData({
                        ...
                        studyGuideUrl: lesson.studyGuideUrl || '',
                ...
                // Fetch available units for parent selection
                const unitsRes = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/products?type=UNIT&isActive=true`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                        'Content-Type': 'application/json'
                    }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const updateData = {
                ...
                studyGuideUrl: formData.studyGuideUrl.trim(),
                digitalUrl: formData.digitalUrl.trim(),
                previewUrl: formData.previewUrl.trim(),
                imageUrl: formData.imageUrl.trim(),
                tags: formData.isFreeLesson ? [] : tags.filter(tag => tag.trim() !== ''),
    ...
    return (
        <div className="p-8">
            <header className="space-y-1">
                ...
                <p className="text-sm text-gray-500">Edit your lesson information and settings.</p>
            </header>
            ...
            <section className="mt-10 space-y-6">
                ...
                <div className="grid gap-6 md:grid-cols-3">
                    ...
                    <div className="md:col-span-2 space-y-5">
                            ...
                            <>
                                <div className="mb-6">
                                    ...
                                    {formData.studyGuideUrl && (
                                        <p className="text-xs text-gray-500 mt-2 break-all">Current: {formData.studyGuideUrl}</p>
                                    )}
                                </div>
                                <div className="mb-6">
                                    <label className="block text-sm font-medium text-gray-700 mb-2">
                                    ...
                                    />
                                    {formData.previewUrl && (
                                        <p className="text-xs text-gray-500 mt-2 break-all">Current: {formData.previewUrl}</p>
                                    )}
                                </div>
                            </>
                        )}
                        ...
                        <div className="mb-6">
                            ...
                            />
                            {formData.digitalUrl && (
                                <p className="text-xs text-gray-500 mt-2 break-all">Current: {formData.digitalUrl}</p>
                            )}
                        </div>
                    </div>
                ...
                <div className="flex justify-end gap-4 pt-6 border-t">
                    <button
                        onClick={() => router.push('/dashboard/course-manage')}
                        disabled={saving}
                        className="rounded-md border border-gray-300 bg-white px-6 py-2 text-sm text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        Cancel
                    </button>
                    <button
