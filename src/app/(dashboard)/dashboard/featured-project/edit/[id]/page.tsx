...
export default function EditPortfolioPage() {
    ...
    useEffect(() => {
        fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/portfolios/${id}`)
            .then(res => res.json())
            .then(data => setFormData(data.data));
    }, [id]);

    const handleUpdate = async () => {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/portfolios/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(formData),
        });

        if (res.ok) router.push('/dashboard/featured-project');
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-4">
                ...
                <h1 className="text-xl font-semibold">Edit Featured</h1>
            </div>
